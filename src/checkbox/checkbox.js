(function() {
    ...
    angular.forEach(['checkbox', 'slider', 'toggle'], function(name, _) {
        ...
        module.directive(name, ['$compile', '$parse', function($compile, $parse) {
            var directive = {
                restrict: 'EA',
                replace: true,
                require: '?ngModel',
    ...
    module
            .controller('RadioGroupCtrl', ['$scope', '$element', 'ngModel', 'onSelect', function RadioGroupCtrl($scope, $element, ngModel, onSelect) {
                    var elements = [], active;
             ...
             */
            .directive('radioGroup', ['$controller', '$parse', function($controller, $parse) {
                    return {
                        restrict: 'EA',
                        require: '?ngModel',
                        link: {
             ...
             *
             */
            .directive('radio', ['$compile', '$parse', '$interpolate', function($compile, $parse, $interpolate) {
                    return {
                        restrict: 'EA',
                        replace: true,
                        require: '^radioGroup',
                        ...
                        link: function(scope, element, attrs, radioGroup) {
                            var myValue = $interpolate(attrs.value || '')(scope),
                                    myLabel = $interpolate(attrs.label || '')(scope),
                                    onSelect = attrs.onSelect && $parse(attrs.onSelect);
                            element[0].value = myValue;
                            element.find('label').text(myLabel);
                            if (onSelect) {
